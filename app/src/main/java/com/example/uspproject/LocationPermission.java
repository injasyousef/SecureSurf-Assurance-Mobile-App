...
public class LocationPermission extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<AppInfo> appsWithLocationPermission = new ArrayList<>();
                ...
                if (packageInfoWithPermissions.requestedPermissions != null) {
                    for (String permission : packageInfoWithPermissions.requestedPermissions) {
                        if (permission.equals(android.Manifest.permission.ACCESS_FINE_LOCATION) ||
                            ...
                            appInfo.hasLocationPermission = true; // Assume initially granted
                            appsWithLocationPermission.add(appInfo);
                            break;
                        }
                    }
                }
        ...
        if (!appsWithLocationPermission.isEmpty()) {
            AppArrayAdapter adapter = new AppArrayAdapter(this, appsWithLocationPermission);
    ...
    private class AppArrayAdapter extends ArrayAdapter<AppInfo> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            Switch switchLocationPermission = convertView.findViewById(R.id.switchbtn);
            ...
            if (appInfo != null) {
                ...
                switchLocationPermission.setChecked(appInfo.hasLocationPermission);

                switchLocationPermission.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    // Update the location permission status
                    appInfo.hasLocationPermission = isChecked;

                    // Handle permission update logic here
                    if (!isChecked) {
                        removeLocationPermission(appInfo.packageName);
                    }
                });
            }
        ...
        private void removeLocationPermission(String packageName) {
            try {
                ...
                Toast.makeText(getContext(), "Please manually revoke location permissions for " + packageName, Toast.LENGTH_LONG).show();
            } catch (Exception e) {
